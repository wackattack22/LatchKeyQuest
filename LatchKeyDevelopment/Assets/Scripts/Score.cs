	...
	void OnGUI() {
		...
		if (currentScene > 1)
		{
			...
			for(int i = 0; i < currentScene-2; i++) {
				...
				GUI.Label(new Rect(Screen.width - 90, j, 100, 20), "Level " + (i+1)
			...
			if (currentScene == lvlScores.Length && PlayerController.lvlComplete)
			{
				...
				GUI.Label(new Rect(Screen.width - 90, j, 100, 20), "Level " + (currentScene)
				...
				j += 20;
				GUI.Label(new Rect(Screen.width - 90, j, 100, 20), "Total Score: " + totalScore.ToString());
				j += 20;
				GUI.Label(new Rect(Screen.width - 90, j, 100, 20), "Rank: " + rank);
			}
